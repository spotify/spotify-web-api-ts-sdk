...
export class SpotifyApi {
  ...
  public async makeRequest<TReturnType>(
    ...
    try {
      const accessToken =
        await this.authenticationStrategy.getOrCreateAccessToken();
  ...
  public async authenticate(): Promise<AuthenticationResponse> {
    ...
    return {
      authenticated:
        response.expires! > Date.now() && !isEmptyAccessToken(response),
  ...
  public static withImplicitGrant(
    clientId: string,
    redirectUri: string,
    scopes: string[] = [],
    config?: SdkOptions
  ...
  public static async performUserAuthorization(
    ...
    const accessToken =
      await client.authenticationStrategy.getOrCreateAccessToken();
