...
export default class BrowseEndpoints extends EndpointsBase {
  ...
  public getCategory(
    ...
    return this.getRequest<Category>(
      `browse/categories/${categoryId}${params}`,
    );
  }
  ...
  public getFeaturedPlaylists(
    ...
    return this.getRequest<FeaturedPlaylists>(
      `browse/featured-playlists${params}`,
    );
  }

  public getPlaylistsForCategory(
    category_id: string,
    country?: CountryCodeA2,
    limit?: MaxInt<50>,
    offset?: number,
    ...
    const params = this.paramsFor({ country, limit, offset });
    return this.getRequest<FeaturedPlaylists>(
      `browse/categories/${category_id}/playlists${params}`,
    );
  }
}
