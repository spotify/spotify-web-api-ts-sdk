...
describe("Integration: Shows Endpoints", () => {
  ...
  it("getShow can return information", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/shows/${valid.id}?market=GB`,
    );
  ...
  it("getShows can return information", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/shows?ids=${valid.id}%2C${valid.id}&market=GB`,
    );
  ...
  it("getShowEpisodes can return information", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/shows/${valid.id}/episodes?market=GB`,
    );
