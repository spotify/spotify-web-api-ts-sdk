...
describe("Integration: Playlists Endpoints", () => {
  ...
  it("getPlaylist can return information", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/playlists/${valid.id}`,
    );
  ...
  it("getPlaylist can return information with additional_types", async () => {
    const valid = validPlaylist();
    const result = await sut.playlists.getPlaylist(
      valid.id,
      undefined,
      undefined,
      ["episode"],
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/playlists/${valid.id}?additional_types=episode`,
    );
    expect(result.tracks.items.length).toBeGreaterThan(0);
  });
  ...
  it("getPlaylistItems can return information", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/playlists/${valid.id}/tracks`,
    );
  ...
  it("getPlaylistItems can return information with additional_types", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/playlists/${valid.id}/tracks?additional_types=episode`,
    );
    expect(result.items.length).toBeGreaterThan(0);
  });
  ...
  it("getUsersPlaylists can return information", async () => {
    ...
    expect(fetchSpy.request(0).input).toBe(
      `https://api.spotify.com/v1/users/${valid.id}/playlists`,
    );
    expect(result.items.length).toBeGreaterThan(0);
  });
