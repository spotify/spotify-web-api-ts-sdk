...
describe(
  "Integration: Users Endpoints (logged in user)",
  () => {
    let sut: SpotifyApi;
    ...
    beforeAll(async () => {
      ...
      wasArtistFollowed = (
        await sut.currentUser.followsArtistsOrUsers([artistId], "artist")
      )[0];
      ...
      wasAlbumSaved = (
        await sut.currentUser.albums.hasSavedAlbums([albumId])
      )[0];
      if (!wasAlbumSaved) {
      ...
      wasAudioBookSaved = (
        await sut.currentUser.audiobooks.hasSavedAudiobooks([audioBookId])
      )[0];
    ...
    it("checkUserFollowsArtistsOrUsers correctly identifies followed artist", async () => {
      ...
      const result = await sut.currentUser.followsArtistsOrUsers(
        [artistId],
        "artist",
      );

      expect(fetchSpy.lastRequest().input).toBe(
        `https://api.spotify.com/v1/me/following/contains?ids=${artistId}&type=artist`,
      );
    ...
    it("create and modify playlists for a user works", async () => {
      ...
      const file = fs.readFileSync("./src/test/valid-image.jpg", {
        encoding: "base64",
      });

      await sut.playlists.addCustomPlaylistCoverImage(result.id, file);
      await sut.playlists.addItemsToPlaylist(result.id, [
        validTrack.uri,
        validTrack.uri,
        validTrack.uri,
        "spotify:track:0ZEigpVOtVunIcimL7dJuh",
      ]);

      const snapshotUpdated = await sut.playlists.movePlaylistItems(
        result.id,
        3,
        1,
        0,
      ); // Move last track to start
    ...
    it("checkCurrentUsersSavedAudiobooks returns true for saved book", async () => {
      const result = await sut.currentUser.audiobooks.hasSavedAudiobooks([
        audioBookId,
      ]);

      expect(fetchSpy.lastRequest().input).toBe(
        `https://api.spotify.com/v1/me/audiobooks/contains?ids=${audioBookId}`,
      );
      expect(result[0]).toBeTruthy();
    });
    ...
    it("hasSavedShow issues correct request for multiple saved shows", async () => {
      ...
      expect(fetchSpy.lastRequest().input).toBe(
        `https://api.spotify.com/v1/me/shows/contains?ids=${showId}%2C${showId}`,
      );
    });
