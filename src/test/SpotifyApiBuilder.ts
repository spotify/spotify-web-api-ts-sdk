...
export function buildIntegrationTestSdkInstance(
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      "No client ID or secret provided. Please provide a valid Spotify client ID and secret in the /.env file as: INTEGRATION_TESTS_SPOTIFY_CLIENT_ID and INTEGRATION_TESTS_SPOTIFY_CLIENT_SECRET",
    );
  }
...
export function buildIntegrationTestUserSdkInstance(
  logResults: boolean = false,
): [SpotifyApi, FetchApiSpy] {
  const clientId = process.env.INTEGRATION_TESTS_SPOTIFY_CLIENT_ID;
  ...
  if (!clientId || !email || !password) {
    throw new Error(
      "No client ID, or secret, or email, or password provided. Please provide a valid Spotify client ID and secret in the /.env file.",
    );
  }

  const authStrat = new AuthAsSpecifcUserForTests(
    clientId,
    Scopes.all,
    email,
    password,
